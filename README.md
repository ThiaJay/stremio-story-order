...
**1.0.14 is the current release candidate.** It combines the current Story Order identity and configure page refresh with the guarded override helper. **1.0.12 remains the last accepted production release until 1.0.14 passes deployment acceptance.** It adds the public capability/status surface without changing Story Order classification, ordering or canonical metadata behaviour. The live Worker bundle was accepted with SHA-256 `cd0e1d8b2e9170b0aa1b73fcece2253f4b27e76680e69f1ead317a8a98960df1` and the public manifest reported 1.0.12. It adds a versioned stable-ID `storyOrder` presentation hint while keeping canonical episode IDs and season/episode coordinates untouched. Existing Stremio clients safely ignore the hint. Compatible clients can display the narrative sequence without reinterpreting watched identity. It hardens the watched-identity safety boundary. The hosted Worker contains no active episode-relocation call path, canonical video coordinates are treated as immutable in safety mode, duplicate or missing stable video IDs fail closed, and the ordering engine remains isolated for future native display-order support. Series episode relocation is temporarily disabled in the hosted service because changing season/episode coordinates can alter Stremio's watched-bitfield and native autoplay interpretation. The service currently passes series video arrays through unchanged while a safe/native alternate-order representation is pursued. The hosted Worker was verified live on 24 September 2026 from source commit `ba66a46f0e38b01470fe017f60b1314c68b894c1`. The deployed bundle matched its validated build hash and the live smoke suite passed after deployment. The hosted configuration page is https://stremio-story-order.storyorder.workers.dev/configure and the default standalone manifest is https://stremio-story-order.storyorder.workers.dev/manifest.json.
