// Production smoke verifies the accepted live 1.0.27 Story Order contract, direct panoramic hero, spacious configure experience, capability status and canonical identity parity.
...
assert.match(liveConfigureHtml,/branding\/v4\/story-order-hero-master\.svg\?v=1\.0\.27/);
assert.match(liveConfigureHtml,/class="hero-master"/);
assert.doesNotMatch(liveConfigureHtml,/class="hero-tiles"/);
assert.match(liveConfigureHtml,/logo\.png\?v=1\.0\.27/);
...
assert.match(liveConfigureHtml,/concept-steps:before\{display:none!important\}/);
...
const liveHeroMasterResponse=await fetch("https://raw.githubusercontent.com/ThiaJay/stremio-story-order/main/public/branding/v4/story-order-hero-master.svg?v=1.0.27",{cache:"no-store"});
...
const liveGlyphResponse=await fetch("https://raw.githubusercontent.com/ThiaJay/stremio-story-order/main/public/branding/v2/story-order-glyph.svg?v=1.0.27",{cache:"no-store"});
...
const liveIconResponse=await fetch("https://raw.githubusercontent.com/ThiaJay/stremio-story-order/main/public/logo.png?v=1.0.27",{cache:"no-store"});
...
for(const [name,size] of [["hero-01.webp",8260],["hero-02.webp",7952],["hero-03.webp",7994],["hero-04.webp",7948],["hero-05.webp",8607]]){
 const response=await fetch("https://raw.githubusercontent.com/ThiaJay/stremio-story-order/main/public/branding/v4/"+name+"?v=1.0.27",{cache:"no-store"});
...
const legacyHeroResponse=await fetch("https://raw.githubusercontent.com/ThiaJay/stremio-story-order/main/public/branding/v2/story-order-order-flow.svg?v=1.0.27",{cache:"no-store"});
