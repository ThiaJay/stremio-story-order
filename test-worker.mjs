...
const canonicalIcon="https://raw.githubusercontent.com/ThiaJay/stremio-story-order/main/public/logo.png?v=1.0.34";
...
assert.match(sourceIcon,/ellipse cx="32" cy="18"/,"source step icon must use the database master");
assert.match(sourceIcon,/transform="translate\(0 7\)"/,"source symbol must be optically lowered inside its viewBox");
const profileStepIcon=await readFile(new URL("./public/branding/v2/step-2-profile.svg",import.meta.url),"utf8");
assert.match(profileStepIcon,/transform="translate\(0 7\)"/,"profile step symbol must be optically lowered inside its viewBox");
const installStepIcon=await readFile(new URL("./public/branding/v2/step-4-install.svg",import.meta.url),"utf8");
assert.match(installStepIcon,/transform="translate\(0 7\)"/,"install step symbol must be optically lowered inside its viewBox");
...
assert.match(html,/branding\/v5\/story-order-hero-approved\.webp\?v=1\.0\.34/);
...
assert.match(html,/concept-num>span\{[\s\S]*line-height:36px!important;[\s\S]*text-align:center!important;[\s\S]*transform:none!important/);
...
assert.match(html,/top:calc\(50% \+ 3px\)!important/,"drawn setup chevrons must share the symbols' optical centre");
...
assert.match(html,/branding\/v5\/example-story-path\.svg\?v=1\.0\.34/);
